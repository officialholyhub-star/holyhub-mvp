...
export default async function MyBusinessPage({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  return <section className="content-narrow">
    ...
    {business && <div className="listing-invite"><div><h2>{business.status === "approved" ? "Your business is approved. Add your products." : "Get your products ready."}</h2><p>{business.status === "approved" ? "Upload photos, set your prices and publish your first listings." : "Save products as drafts while your application is reviewed. Publishing opens after approval."}</p></div><Link className="button button-primary" href="/seller/products">Manage products →</Link></div>}

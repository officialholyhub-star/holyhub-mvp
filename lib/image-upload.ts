...
export async function sanitiseImage(file:File){
 if(!["image/jpeg","image/png","image/webp"].includes(file.type)||file.size===0||file.size>4_000_000)throw new Error("Choose a JPG, PNG or WebP image no larger than 4 MB.");

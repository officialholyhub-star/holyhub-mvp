...
import { all, productSelect, visibleProducts } from "@/lib/data";
import { productCategories, type Product } from "@/lib/data";
import { ProductCard } from "@/components/product-card";
import { DiscoveryPaths } from "@/components/discovery-paths";

export default async function HomePage() {
  let featured: Product[] = [], featuredError = false;
  try { featured = await all<Product>(productSelect + " WHERE " + visibleProducts + " ORDER BY p.created_at DESC LIMIT 4"); } catch { featuredError = true; }

...
test('responsive discovery, real image access, filters and unpaid multi-seller basket',async({page,request})=>{
 ...
 await page.getByLabel('Search products').fill('Hope');await page.getByRole('button',{name:'Find products'}).click();await expect(page.getByText('1 products to discover')).toBeVisible();
...
test('seller saves, uploads, publishes, edits and archives a product',async({page,browser})=>{
 ...
 await page.getByLabel('Add a photo').setInputFiles({name:'journal.png',mimeType:'image/png',buffer:await png()});await page.getByRole('button',{name:'Upload photo',exact:true}).click();await expect(page.getByText('Image uploaded.',{exact:true})).toBeVisible();await page.getByRole('button',{name:'Publish product'}).click();await expect(page.getByRole('link',{name:'View product'})).toBeVisible();
 const context=await browser.newContext(),visitor=await context.newPage();await visitor.goto('http://127.0.0.1:3102/products?q=New');await expect(visitor.getByRole('heading',{name:'New faith journal',exact:true})).toBeVisible();
 ...
 await page.goto(productUrl);await page.getByRole('button',{name:'Archive product'}).click();await visitor.reload();await expect(visitor.getByRole('heading',{name:'New faith journal',exact:true})).toHaveCount(0);await context.close();
});
